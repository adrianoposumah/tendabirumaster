...
if (isset($_POST['UpdateStaff'])) {
  ...
  if (empty($_POST["staff_number"]) || empty($_POST["staff_name"]) || empty($_POST['staff_email']) || empty($_POST['staff_password'])) {
    ...
    $postQuery = "UPDATE staffs SET  staff_number =?, staff_name =?, staff_email =?, staff_password =? WHERE staff_id =?";
    ...
    if ($postStmt) {
      $success = "Staff Updated" && header("refresh:1; url=staff.php");
    } else {
...
<body>
  ...
  <div class="main-content">
    ...
    $update = $_GET['update'];
    $ret = "SELECT * FROM  staffs WHERE staff_id = '$update' ";
